...
impl AsyncDataApi {
    pub async fn new(
        ...
        let mut client_builder = AsyncClientBuilder::new();
        ...
        Ok(Self {
            agent,
            ddragon: client_builder.build().await?,
    ...
    pub async fn get_champ_data(&self) -> Result<HashMap<String, ChampionShort>, UggError> {
        Ok(self.ddragon.champions().await?.data)
    }

    pub async fn get_items(&self) -> Result<HashMap<String, Item>, UggError> {
        Ok(self.ddragon.items().await?.data)
    }

    pub async fn get_runes(&self) -> Result<HashMap<i64, RuneExtended<RuneElement>>, UggError> {
        let rune_data = self.ddragon.runes().await?;
    ...
    pub async fn get_summoner_spells(&self) -> Result<HashMap<i64, String>, UggError> {
        let summoner_data = self.ddragon.summoner_spells().await?;
...
impl AsyncUggApi {
    pub async fn new(
        ...
        let mut inner_api = AsyncDataApi::new(version, cache_dir.clone()).await?;

        let mut current_version = inner_api.get_current_version();
        let allowed_versions = inner_api.get_supported_versions().await?;
        let ugg_api_versions = inner_api.get_ugg_api_versions().await?;
        ...
        if let Some(default_if_fails) = versions_ugg_supports.first() {
            ...
            {
                inner_api =
                    AsyncDataApi::new(Some(default_if_fails.ddragon.clone()), cache_dir).await?;
        ...
        let champ_data = inner_api.get_champ_data().await?;
        let items = inner_api.get_items().await?;
        let runes = inner_api.get_runes().await?;
        let summoner_spells = inner_api.get_summoner_spells().await?;
...
pub struct AsyncUggApiBuilder {
    version: Option<String>,
    cache_dir: Option<PathBuf>,
}

impl AsyncUggApiBuilder {
    pub fn new() -> Self {
        Self {
            version: None,
            cache_dir: None,
    ...
    pub async fn build(self) -> Result<AsyncUggApi, UggError> {
        AsyncUggApi::new(self.version, self.cache_dir).await
    }
}

impl Default for AsyncUggApiBuilder {
    fn default() -> Self {
        Self::new()
    }
}
